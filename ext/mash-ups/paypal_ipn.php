...
class PaypalIpn {
    ...
    public function __construct($sandbox=TRUE) {
        ...
        $this->url = $sandbox ? self::SANDBOX_URL : self::URL; //'https://www.paypal.com/cgi-bin/webscr';
        $this->last_error = '';
        ...
        $this->rm = '2';           // Return method = POST
        $this->cmd = '_xclick';
    }

    public function __set($var, $val) { $this->fields[$var] = $val; }

    public function __get($var) {
        return isset($this->{$var}) ? $this->{$var} :
               (isset($this->fields[$var]) ? $this->fields[$var] : NULL);
    }
