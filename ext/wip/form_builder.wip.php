...
class FormBuilder {
    ...
    public static function model(Model $model, $customizers=array()) {
        ...
        if (!isset($customizers[':exclude_xrefs']) || (isset($customizers[':exclude_xrefs']) && $customizers[':exclude_xrefs'] === FALSE)) {
                ...
                foreach ($associates as $name => $associate) {
                    if (!isset($customizers[':excluded_xrefs']) || (is_array($customizers[':excluded_xrefs']) && !in_array($name, $customizers[':excluded_xrefs']))) {
                        ...
                        if ($associate->type == 'has_and_belongs_to_many') {
                            ...
                            if ($model->_persisted) {
                                $form->addFieldset(self::fieldset($model[$associate->name], $customizers));
                            } else {
                                $o = new $aclass;
                                $o->ref_model = $model->_table;
                                $fs = self::fieldset($o, $customizers);
                                $form->addFieldset($fs);
                            }
                        }
                    }
    ...
    public static function fieldset(Model $model, $customizers=array()) {
        $form   = new Partial('fieldset');
        $fields = '';
        $table  = $model->getTable();
        $id     = $model->getId();
        $mclass = get_class($model);
        $form->legend  = String::titleize($mclass)->to_s;        
        $map_details = array();
        foreach ($table as $field) {
            if (isset($customizers[':exclude']) && is_array($customizers[':exclude']) && in_array($field->name, $customizers[':exclude'])) {
                continue;
                ...
                foreach ($associates as $name => $associate) {
                    if (!isset($customizers[':excluded_xrefs']) || (is_array($customizers[':excluded_xrefs']) && !in_array($name, $customizers[':excluded_xrefs']))) {
                        ...
                        if ($associate->type == 'has_and_belongs_to_many') {
                            ...
                            if ($model->_persisted) {
                                $fields .= self::fieldset($model[$associate->name], $customizers);
                            } else {
                                $o = new $aclass;
                                $o->ref_model = $model->_table;
                                $fs = self::fieldset($o, $customizers);
